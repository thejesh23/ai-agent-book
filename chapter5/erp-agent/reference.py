"""
Independent Python reference implementation: directly compute the expected answers for 10 questions on seed data (in-memory list).

These functions deliberately avoid SQL, and are used to verify the correctness of SQL generated by the Agent.
Each function returns list[tuple], where the column order in the tuple is consistent with the
"column order hint" given to the Agent in questions.py, facilitating row-by-row comparison.
"""

from datetime import date
from statistics import mean

DEPT_A = "R&D Department"   # "Department A" in the question
DEPT_B = "Sales Department"   # "Department B" in the question
...
def q3_dept_highest_avg_level(emps, sals, today):
    ...
    return [(top[0],)]  # Return only department name
...
def q5_deptA_avg_salary_range(emps, sals, today):
    y = today.year
    lo, hi = (y - 2, 3), (y - 1, 5)  # March of the year before last ~ May of last year (inclusive)
...
def q8_avg_latest_salary_by_tenure(emps, sals, today):
    buckets = {" Within one year of joining": [], " One to two years": [], " Two to three years": []}
    for e in emps:
        days = (today - e["hire_date"]).days
        if days < 365:
            b = " Within one year of joining"
        elif days < 730:
            b = " One to two years"
        elif days < 1095:
...
# Question number -> reference implementation
REFERENCE = {
