...
6.  **The Agent Itself**—using code to create new Agents and complete a bootstrap loop (distinct from the "self-evolution" in Chapter 8 that does not change weights).
...
First, we need to distinguish this topic from Chapter 8. This section concerns Agents that use code to **repair themselves and create other Agents**—through self-repair, self-replication, and on-demand creation—with Agent code and architecture as the objects of change. Chapter 8's "self-evolution" is different: it concerns an Agent's ability to expand its capabilities continuously by accumulating experience, optimizing prompts, and building a tool library **without modifying model weights**; its targets are knowledge and strategy. Both processes can be described as "evolution," but to avoid confusion with Chapter 8's title, this section uses **bootstrapping** for the capability to produce Agent code.
...
We have now covered two of the three pillars, context and tools—and code generation is the most versatile tool of all. But one key question remains: how can we scientifically measure whether these design decisions work? In the next chapter, we turn to the third pillar—the model—beginning with evaluation. That chapter develops a complete evaluation methodology, covering evaluation environments, dataset design, reward models, and evaluation-driven model selection, so that every technique discussed thus far can be validated quantitatively.
