...
async def run_memory_op(agent: Mem0Agent, args) -> None:
    ...
    op = args.op
    if not op:
        console.print("[red]memory mode requires --op parameter (add/search/get-all/history/delete)[/red]")
        sys.exit(1)
    ...
    if op == "add":
        if not args.text:
            console.print("[red]add operation requires --text parameter (a conversation text or JSON message file path)[/red]")
            sys.exit(1)
        messages = _load_add_messages(args.text)
        events = await asyncio.to_thread(agent.add_memory, messages, args.user_id, args.agent_id)
        console.print(f"[green]Write completed, events generated by memory pipeline (extract—compare—decide):[/green]")
        if events:
            for ev in events:
                console.print(f"  [{ev['event']}] {ev['memory']}  [dim](id={ev['id']})[/dim]")
        else:
            console.print("  [dim](No ADD/UPDATE/DELETE — candidate facts judged as NOOP duplicates)[/dim]")
        result = {"op": "add", "user_id": args.user_id, "events": events}

    elif op == "search":
        if not args.query:
            console.print("[red]search operation requires --query parameter[/red]")
            sys.exit(1)
        hits = await asyncio.to_thread(agent.search_memory, args.query, args.user_id, args.agent_id)
        console.print(f"[green]Retrieved {len(hits)} related memories:[/green]")
        for mem in hits:
        ...
        console.print(f"[green]User {args.user_id} has {len(memories)} memories:[/green]")
        ...
        if not args.memory_id:
            console.print("[red]history operation requires --memory-id parameter[/red]")
            sys.exit(1)
        history = await asyncio.to_thread(agent.memory_history, args.memory_id)
        console.print(f"[green]Modification history of memory {args.memory_id}:[/green]")
        ...
        if not args.memory_id:
            console.print("[red]delete operation requires --memory-id parameter[/red]")
            sys.exit(1)
        await asyncio.to_thread(agent.delete_memory, args.memory_id)
        console.print(f"[green]Deleted memory {args.memory_id}[/green]")
...
CLI_EPILOG = """\
Examples:
  python main.py                              # Default: interactive conversation (memory auto-write/retrieve)
  python main.py --mode demo --user-id u1     # Run "Beijing→Shanghai" memory pipeline demo (ADD/UPDATE/DELETE/NOOP)
  python main.py --mode memory --op add   --text "I live in Beijing and am a backend engineer" --user-id u1
...
Note: memory / demo / interactive / batch / benchmark all require a usable LLM API (KIMI_API_KEY)
and vector storage; Mem0's memory extraction and retrieval depend on online model calls.
"""
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Mem0 Memory Agent (Kimi K3) — Demonstrates Mem0's \"extract—compare—decide\" memory pipeline",
    ...
    parser.add_argument(
        ...
        default="interactive",
        help="Run mode: interactive conversation (default) / batch conversation / benchmark LOCOMO benchmark / "
             "memory direct memory operations / demo memory pipeline demo",
    )
    parser.add_argument(
        "--op",
        choices=["add", "search", "get-all", "history", "delete"],
        help="Memory operations in memory mode: add write / search retrieve / get-all list all / "
             "history view modification history of a memory / delete delete",
    )
    parser.add_argument("--text", type=str,
                        help="Conversation input for add operation: a text, or path to a JSON message list file")
    parser.add_argument("--query", type=str, help="Query statement for search operation")
    parser.add_argument("--memory-id", type=str, help="Memory ID for history / delete operations")
    parser.add_argument("--user-id", type=str, default="user_001",
                        help="User ID for memory ownership (default user_001)")
    parser.add_argument("--agent-id", type=str, default="agent_001",
                        help="Agent ID (default agent_001)")
    parser.add_argument("--model", type=str,
                        help="Override MODEL_NAME, specify conversation model (e.g., kimi-k3)")
    parser.add_argument("--input", type=str, help="Input JSON file for batch mode")
    parser.add_argument("--output", type=str,
                        help="JSON file to write results to (memory / batch mode)")
    parser.add_argument("--config", type=str, help="Configuration file path (reserved)")
